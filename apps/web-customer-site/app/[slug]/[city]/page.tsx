...
export default async function ServiceCityPage(props: PageProps) {
  ...
  // Filter salons by service and city
  ...
  return (
    <StandardPageLayout>
        <Link href={`/${slug}`} className="inline-flex items-center text-sm text-muted-foreground hover:text-primary mb-6">
        ...
        <p className="text-muted-foreground mb-8">
            Najlepsi specjaliści ({service.name.toLowerCase()}) w lokalizacji {city.name}.
        </p>
        
        <ListView 
            salons={filteredSalons} 
            title={`Salony: ${service.name} - ${city.name}`}
            subtitle={filteredSalons.length > 0 
